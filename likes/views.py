...
def like(request, content_type, id, vote):
    ...
    if request.is_ajax():
        return views.vote(request, content_type=content_type, object_id=id, vote=vote, template_name='likes/inclusion_tags/likes.html', can_vote_test=can_vote, extra_context={'can_vote': False, "content_type": url_friendly_content_type})
    else:
